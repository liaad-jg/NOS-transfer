...
def finished_trajectories_query(day, time_start, time_end, raw_data_table, project_dataset, braga_poly, povoa_poly):
    query = f"""WITH raw_data_table AS (
                    ...
                    WHERE 
                        ...
                        rec_timestamp >= COALESCE(unfinished.first_timestamp, '{time_start}') AND rec_timestamp < '{time_end}' AND
                        (ST_INTERSECTS(ST_GEOGFROMTEXT('{braga_poly}'), ST_GEOGPOINT(CAST(lon AS FLOAT64) , CAST(lat AS FLOAT64)))
                        OR ST_INTERSECTS(ST_GEOGFROMTEXT('{povoa_poly}'), ST_GEOGPOINT(CAST(lon AS FLOAT64) , CAST(lat AS FLOAT64))))
                ),
                logs_before_gap AS (
                    SELECT
                        imsi,
...
def unfinished_trajectories_query(day, time_start, time_end, raw_data_table, project_dataset, braga_poly, povoa_poly):
